...
// CASES DE SUCESSO & SISTEMAS (DEEP TECH, OPEN SOURCE E PRODUTOS EM PRODUÇÃO)
export const DEFAULT_PROJECTS = [
  ...
  },
  {
    ...
    hero_image: "",
    gallery: [],
    tags: ["Rust", "Linux Kernel", "BPF LSM", "Kernel Customizado WSL2", "Contenção de IA", "Open Source"],
  ...
  },
  {
    ...
    slug: "greentoken",
    title: "GreenToken",
    badge: "FinOps & Observabilidade de IA · Paper Publicado no Zenodo",
    summary: "Medidor de eficiência energética e FinOps para clusters de IA. Calcula o custo real em Watts, Joules e dólares por token gerado através de instrumentação no silício (RAPL, NVML e eBPF). Pesquisa científica com dados brutos e artigo publicados no Zenodo.",
    problem_description: "A explosão dos servidores de inferência de inteligência artificial (vLLM, Ollama, TGI, llama.cpp) trouxe consigo um desafio financeiro de alta magnitude: contas astronômicas de infraestrutura em nuvem e energia elétrica.\n\nToda liderança técnica e time de FinOps recebe a fatura agregada do data center no encerramento do mês, mas praticamente ninguém tem a resposta para a métrica mais crucial do negócio: **quanto custa, em Joules físicos e em dólares, cada resposta de 1.000 tokens gerada pelo seu modelo?**\n\nAs ferramentas tradicionais de observabilidade apenas informam o consumo elétrico do servidor como um todo. Se três modelos de IA dividem o mesmo computador com GPUs, é impossível saber qual deles é responsável pelo consumo desproporcional. O GreenToken foi concebido para resolver essa lacuna, atribuindo o consumo elétrico exato por processo (PID) e por fluxo de tokens gerados.",
    technical_details: "### 1. Artigo Científico e Publicação Independente no Zenodo\nA formulação do GreenToken é sustentada por metodologia científica formal (GT-M), com hipóteses pré-registradas em controle de versão, código de coleta público e dados brutos auditáveis hospedados com identificador persistente no repositório do CERN (Zenodo):\n\n> **Citação Oficial do Artigo:**\n> Freire, R. (2026). *GT-M — Medição Energética Independente para Inferência de IA: Decomposição por Fase, Fronteira de Pareto Acurácia-Energia e Energia Amortizada por Ciclo de Trabalho (com apêndice sobre deriva simulada)*. Zenodo.  \n> **DOI Permanente:** [10.5281/zenodo.22036311](https://doi.org/10.5281/zenodo.22036311)\n\n### 2. A Série Experimental GT-M (E1, E2, E3 e E4)\nO rigor da metodologia GT-M foi consolidado ao longo de quatro experimentos em hardware real:\n\n- **Experimento E1 (Decomposição por Fase):** Comprovação prática no silício da escala quadrática do mecanismo de atenção ($O(N^2)$) de acordo com o tamanho do contexto (128 a 1024 tokens), convergindo com a previsão teórica.\n- **Experimento E2 (Fronteira de Pareto Acurácia-Energia):** Teste de precisão numérica com acurácia controlada (FP32 a 51,68 J/inf, FP16 a 19,10 J/inf e INT8 a 34,00 J/inf), comprovando que o FP16 entrega 63,04% de economia de energia com dispersão CV entre 2,6% e 3,7%.\n- **Experimento E3 (Deriva de Simulação):** Apêndice metodológico mantido expressamente separado para isolar simulações matemáticas de dados físicos medidos em silício.\n- **Experimento E4 (Ciclo de Trabalho e Energia Amortizada):** O estudo de campo mais impactante sobre a operação de IA em regime de carga realista.\n\n### 3. SEÇÃO ESPECIAL: A Descoberta Central do Experimento E4 (Duty Cycle & Degradação de 12.08x)\nNo fechamento da série experimental, o **Experimento E4** instrumentou uma GPU NVIDIA Tesla T4 submetida à mesma quantidade de trabalho útil (20 inferências normalizadas conforme a trava do Gate G4.3) sob quatro perfis de utilização operacional:\n\n| Perfil de Carga | Regime Operacional | Duração Janela | Energia Total | **Energia Amortizada por Inferência Útil** | Fator vs Pico |\n|---|---|---|---|---|---|\n| **Profile_100pct_Saturada** | 100% Carga (Pico) | 8.7 s | 589.00 J | **29.45 J / inf útil** | **1.00x (Número de Folheto)** |\n| **Profile_50pct_Alta** | 50% Carga / 50% Idle | 17.9 s | 956.65 J | **47.83 J / inf útil** | **1.62x de degradação** |\n| **Profile_20pct_Media** | 20% Carga / 80% Idle | 48.3 s | 2124.69 J | **106.23 J / inf útil** | **3.61x de degradação** |\n| **Profile_5pct_Baixa** | 5% Carga / 95% Idle | 200.6 s | 7117.42 J | **355.87 J / inf útil** | **12.08x de degradação!** |\n\n#### A Física por Trás do Fenômeno: Histerese de Contexto CUDA vs Repouso Frio\nA modelagem teórica ingênua presumia que, nas pausas entre requisições de baixa carga, a GPU descansaria no seu estado de sono mais profundo (P-State P8 consumindo apenas 9.9 Watts), o que geraria uma estimativa de 124,45 J por inferência (fator de 4.23x).\n\nNo entanto, a medição empírica revelou **7117.42 Joules consumidos — ou seja, 355.87 Joules por inferência útil, uma degradação real de 12.08 vezes**.\n\nA auditoria física explicou a discrepância: em servidores de produção real, o driver da GPU mantém o contexto de execução CUDA permanentemente residente na memória VRAM para não pagar a penalidade de recarregar gigabytes de pesos a cada chamada. Esse contexto residente impede que o hardware entre em repouso profundo, mantendo a GPU presa em estados P0/P2 com **consumo estático de 34.02 Watts contínuos**.\n\n> **Conclusão Científica:** O custo real de manter um modelo de inteligência artificial em produção não reside apenas no processamento ativo, mas na marcha lenta de prontidão. Em regimes de baixa demanda, o consumo por entrega útil é até doze vezes superior ao anunciado nos testes sintéticos de laboratório.\n\n### 4. Arquitetura de Coleta & Princípios Invioláveis\n- **Instrumentação Direta no Silício:** Medição de Watts na CPU e DRAM via registradores Intel/AMD RAPL; medição de GPU em tempo real via biblioteca NVML da NVIDIA.\n- **Correlação Temporal por eBPF:** Rastreia o escalonador do kernel Linux (`sched_switch`) para mapear com precisão de microssegundos o tempo de processamento consumido por cada PID.\n- **Contador Canônico de Tokens:** Integração direta com os endpoints `/metrics` dos motores de inferência (vLLM), atestando 0.0% de erro em concorrência.\n- **Zero Overhead & Totalmente Passivo:** O GreenToken nunca altera cgroups, nunca reduz frequências e nunca interfere no SLA de entrega do servidor.",
    cover_image: "",
    hero_image: "",
    gallery: [],
    tags: ["FinOps de IA", "NVIDIA NVML", "Intel/AMD RAPL", "eBPF", "Paper no Zenodo", "Open Source"],
    github_url: "https://github.com/rodrigoffreir3",
    live_url: "",
    is_featured: true,
    display_order: 3
  },
  {
    ...
    badge: "Engenharia de Kernel Linux · Microsoft WSL2",
    summary: "Kernel Linux customizado para Windows (WSL2) compilado da árvore oficial da Microsoft com suporte completo a BPF LSM, CONFIG_SECURITY_BPF e interfaces BTF, viabilizando segurança determinística em Ring 0 e validação de ferramentas de IA no Windows.",
    problem_description: "O subsistema WSL2 da Microsoft é a ferramenta padrão de milhões de engenheiros para executar ambientes Linux sobre o Windows. No entanto, a Microsoft compila a imagem padrão do kernel distribuída no sistema com os módulos avançados de segurança Linux Security Module (BPF LSM) desligados, além de desativar os metadados BTF necessários para programas modernos de eBPF.\n\nEssa restrição impedia desenvolvedores e pesquisadores de testar ferramentas avançadas de contenção e defesa em Ring 0 (como o SysCallCage e programas LSM preventivos) de forma nativa no Windows, forçando a montagem de máquinas virtuais pesadas ou a dependência de instâncias remotas em nuvem.",
    technical_details: "### 1. Compilação a partir da Árvore Oficial da Microsoft\nO projeto estabelece um pipeline estruturado e auditável para compilar o repositório oficial `WSL2-Linux-Kernel` mantido pela Microsoft, aplicando as flags indispensáveis de segurança no arquivo de configuração do kernel:\n\n- `CONFIG_BPF_LSM=y` — habilita pontos de interceptação síncrona de segurança governados por eBPF;\n- `CONFIG_SECURITY_BPF=y` — ativa o subsistema de segurança BPF acoplado ao Linux Security Module;\n- `CONFIG_DEBUG_INFO_BTF=y` — compila as informações de tipo BTF (BPF Type Format) essenciais para a portabilidade de programas eBPF CO-RE (Compile Once, Run Everywhere).\n\n### 2. Resolução de Falhas no Verifier do Kernel\nDurante os testes de integração do SysCallCage no WSL2, foram identificadas falhas de rejeição no verifier do kernel Linux ao inspecionar caminhos aninhados de descritores de arquivos em tempo de execução. O projeto documenta a análise profunda do código C do kernel, a correção dos acessos a ponteiros de estruturas aninhadas e a compilação com Clang/LLVM sem comprometer a estabilidade do Windows.\n\n### 3. Diretriz Zero-Trust na Cadeia de Suprimentos\nEm conformidade estrita com as boas práticas de segurança, o projeto rejeita a disponibilização de binários de kernel pré-compilados por terceiros. Toda a documentação fornece scripts transparentes para que o próprio desenvolvedor clone os fontes da Microsoft, valide assinaturas e compile seu kernel localmente com total soberania e auditabilidade.",
    cover_image: "",
    hero_image: "",
    gallery: [],
    tags: ["Linux Kernel", "Microsoft WSL2", "BPF LSM", "Clang/LLVM", "Engenharia de Baixo Nível"],
    github_url: "https://github.com/rodrigoffreir3",
    live_url: "",
    is_featured: false,
    display_order: 4
  },
  {
    ...
    slug: "creare-labs",
    title: "CreareLabs",
    badge: "Simulação Digital Twin & Hardware · WebAssembly",
    summary: "Plataforma de Gêmeo Digital (Digital Twin a 20Hz) e robótica educacional que executa 100% no navegador via WebAssembly. Possibilita simulação com física 2D precisa, programação dual e comissionamento de placas ESP32 via Sim-to-Real.",
    problem_description: "O ensino e desenvolvimento prático de automação, robótica e eletrônica sempre enfrentaram barreiras de custo e logística: equipamentos caros, queima frequente de componentes eletrônicos por erros de ligação e dificuldades de instalação de drivers e softwares proprietários pesados em computadores escolares ou modestos.\n\nO CreareLabs soluciona esse gargalo transportando o laboratório inteiro para dentro do navegador web, permitindo que estudantes e engenheiros criem circuitos, programem a lógica e testem em simulação física de alta fidelidade antes de enviar o código para placas físicas reais.",
    technical_details: "### 1. Digital Twin em Tempo Real (20Hz)\nA plataforma sincroniza o gêmeo virtual a uma cadência estável de 20 atualizações por segundo (20Hz), modelando com precisão o comportamento contínuo de sensores de distância, temperatura, atuadores mecânicos, motores e esteiras transportadoras.\n\n### 2. Motor de Física 2D Ultraleve e Preciso\nIntegração de um motor de física bidimensional determinístico (Matter.js) calibrado para simular gravidade, atrito, colisões e forças cinéticas com baixíssimo consumo de memória, assegurando resposta imediata e gráficos fluidos mesmo em computadores básicos.\n\n### 3. Ambiente de Programação Duplo\nA plataforma unifica duas formas de programar em um único fluxo integrado:\n\n- **Folha de Eventos Visual:** Programação lógica baseada em condições e ações, perfeita para a introdução ao raciocínio lógico e robótica.\n- **Editor Profissional Monaco:** Ambiente de desenvolvimento com syntax highlighting, autocompletion e validação de regras para código Python real.\n\n### 4. Execução WebAssembly com Proteção AST Guard\nTodo o código Python é executado localmente na máquina do usuário através de WebAssembly (Pyodide), sem necessidade de servidores de backend para compilação. Para garantir que códigos com laços infinitos (`while True`) não congelem a aba do navegador, o motor conta com um analisador de Árvore Sintática Abstrata (AST Guard) que injeta chamadas cooperativas de rendimento de tempo de CPU.\n\n### 5. Pipeline Sim-to-Real via cURL e USB (ESP32)\nUma vez validado no simulador virtual, o mesmo algoritmo é gravado diretamente em microcontroladores físicos ESP32 rodando MicroPython através de conexões seriais USB (WebSerial) ou chamadas diretas via cURL, completando o ciclo da simulação digital para a aplicação física real.",
    cover_image: "",
    hero_image: "",
    gallery: [],
    tags: ["Digital Twin 20Hz", "WebAssembly (WASM)", "Sim-to-Real (ESP32)", "Física 2D", "Python no Navegador"],
    github_url: "https://github.com/rodrigoffreir3",
    live_url: "",
    is_featured: false,
    display_order: 5
  },
  {
    ...
    slug: "appfitness",
    title: "AppFitness",
    badge: "Plataforma SaaS White-Label · Em Produção Ativa",
    summary: "Plataforma SaaS white-label para gestão de treinos e retenção de alunos. Em produção ativa com cerca de 100 usuários reais, navegação mobile-first a 60fps e distribuição global de mídia em CDN Cloudflare R2.",
    problem_description: "A alta evasão de alunos em academias e estúdios está fortemente vinculada à frustração com aplicativos tradicionais: pesados, lentos para abrir durante o treino, poluídos com banners e desconfortáveis para manuseio com uma mão só entre uma série de exercícios e outra.\n\nO AppFitness foi projetado com uma obsessão ergonômica mobile-first: fornecer ao aluno uma experiência fluida e instantânea para acompanhar treinos e cargas, enquanto oferece aos gestores indicadores precisos de assiduidade para resgatar alunos antes do cancelamento da matrícula.",
    technical_details: "### 1. Operação Real em Produção\nNão se trata de um conceito visual: a plataforma está em operação ativa diária atendendo a uma base consolidada de aproximadamente 100 alunos reais e personal trainers, com alta taxa de adesão diária de check-ins de treino.\n\n### 2. Arquitetura Multi-Tenant White-Label\nProjetada para que estúdios, boxes de crossfit e personal trainers utilizem a plataforma sob sua própria marca, com personalização de paleta visual, logos e regras de treino mantendo isolamento estrito de dados entre clientes sob a mesma infraestrutura em nuvem.\n\n### 3. Ergonomia Mobile-First a 60fps\nDesign refinado para operação em smartphones com uma mão só, transições visuais fluidas travadas a 60 quadros por segundo, tempos de carregamento instantâneos e capacidade de anotação de cargas com sincronização offline para ambientes de academia com sinal celular instável.\n\n### 4. Mídia e Vídeos em CDN Cloudflare R2\nArmazenamento e entrega de demonstrações em vídeo de alta definição para cada movimento de treino distribuídos através da rede de borda da Cloudflare (R2), garantindo início imediato de reprodução e custo zero de transferência de dados (egress).\n\n### 5. Inteligência de Retenção e Assiduidade\nPainel dedicado para os treinadores que detecta automaticamente desvios na frequência de treinos dos alunos, emitindo notificações preventivas para contato ativo com o aluno antes que o afastamento culmine em evasão.",
    cover_image: "",
    hero_image: "",
    gallery: [],
    tags: ["SaaS White-Label", "Produção Ativa (~100 Alunos)", "Mobile-First 60fps", "Cloudflare R2 CDN", "PWA"],
    github_url: "https://github.com/rodrigoffreir3",
    live_url: "",
    is_featured: false,
    display_order: 6
  },
  {
    id: 7,
    slug: "oficial-helper",
    title: "Oficial Helper",
    badge: "PWA Móvel de Alta Precisão · Oficialato de Justiça",
    summary: "Aplicativo móvel progressivo desenvolvido para oficiais de justiça salvarem coordenadas geográficas de alta precisão por GPS em áreas rurais e ramais sem CEP, vinculadas a mandados judiciais, partes e contatos com suporte offline total.",
    problem_description: "O cumprimento de ordens e mandados judiciais em comarcas com extensas zonas rurais enfrenta uma barreira geográfica diária: estradas vicinais de terra, ramais, glebas e propriedades rurais que não possuem código postal (CEP), nome de rua formal ou numeração oficial.\n\nHistoricamente, a localização de uma residência rural dependia de anotações informais em papéis avulsos ou memória do oficial ('após o terceiro mata-burro à direita'). Em reiterações de mandados, substituições por férias ou diligências de outros servidores, perdia-se tempo excessivo tentando reencontrar a parte, sem contar a exigência legal de certificar com fidelidade a localização geográfica do ato praticado.",
    technical_details: "### 1. Captura GPS de Alta Precisão Geodésica\nO sistema acessa diretamente os sensores de posicionamento por satélite do smartphone, coletando latitude e longitude em graus decimais, com registro explícito da precisão estimada em metros e altitude no momento exato da diligência.\n\n### 2. Cadastro Estruturado Vinculado ao Mandado Judicial\nCada coordenada salva no mapa é estruturada com as informações processuais indispensáveis:\n\n- Nome completo da parte citada ou intimada;\n- Contatos telefônicos de referência obtidos em campo;\n- Número do processo judicial e número do mandado;\n- Descrição detalhada do acesso físico e pontos de referência terrestres.\n\n### 3. Filtros Estratégicos por Localidade e Otimização de Rotas\nAgrupamento automático de mandados por comarca, gleba, ramal, linha ou setor geográfico. Essa funcionalidade viabiliza o planejamento de viagens e o cumprimento de múltiplos mandados em uma única rota planejada, gerando economia expressiva de combustível e tempo.\n\n### 4. Operação Offline com Rascunhos Locais\nProjetado com arquitetura offline-first indispensável para o trabalho de campo em regiões isoladas onde a rede celular inexiste. Todos os dados são persistidos no armazenamento local do dispositivo e sincronizados de maneira segura assim que o sinal de rede é restabelecido.\n\n### 5. Roadmap: Foto Georreferenciada com Timestamp Inviolável\nEm desenvolvimento no ciclo técnico, o aplicativo integrará captura fotográfica diretamente da câmera com estampagem indelével de data, hora precisa e coordenadas geográficas sobrepostas na imagem, servindo de elemento de fé pública para anexação direta às certidões judiciais de cumprimento de mandado.",
    cover_image: "",
    hero_image: "",
    gallery: [],
    tags: ["PWA Mobile", "GPS Alta Precisão", "Zona Rural Sem CEP", "Gestão de Mandados", "Operação Offline"],
    github_url: "https://github.com/rodrigoffreir3",
    live_url: "",
    is_featured: false,
    display_order: 7
